...
function init()
    ...
    --modem.transmit(4, 4,"chassis control computer init complete")
...
function attitude_calc_task()
    ...
    if attitude.angle.yaw-attitude.last_yaw > pi then
        attitude.omega.y = (attitude.angle.yaw-attitude.last_yaw-2*pi)/time_piece
    elseif attitude.angle.yaw-attitude.last_yaw < -pi then
        attitude.omega.y = (attitude.angle.yaw-attitude.last_yaw+2*pi)/time_piece
    else
...
function message_receive_task()
    ...
    if senderChannel == 1 and message ~= nil then
        --print(message)
        msg.left_spd,msg.right_spd,msg.gimbal_mode,msg.gimbal_yaw_ang,msg.gimbal_pitch_ang,msg.gimbal_firepermit = message:match("(-?%d+) (-?%d+) (-?%d+) (-?%d+) (-?%d+) (-?%d+)")
        --msg.left_spd,msg.right_spd = message:match("(-?%d+) (-?%d+)")
        parameter.left_last_tar_spd = parameter.left_tar_spd
        parameter.right_last_tar_spd = parameter.right_tar_spd
        parameter.left_tar_spd = msg.left_spd
        parameter.right_tar_spd = msg.right_spd
        ...
        print(msg.gimbal_yaw_ang)
    end
end

function message_send_task()
    modem.transmit(1,1,attitude.omega.y)
end
...
init()
while true do
    parallel.waitForAll(chassis_control_task, message_receive_task,attitude_calc_task,message_send_task)
end