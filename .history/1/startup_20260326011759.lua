...
function gimbal_remote_get()
    ...
    if keyboard.button.left then
        gimbal.yaw_ang = gimbal.yaw_ang - ang_step
    end
    ...
    if keyboard.button.up then
        gimbal.pitch_ang = gimbal.pitch_ang - ang_step
    end

    if keyboard.button.down then
        gimbal.pitch_ang = gimbal.yaw_ang - ang_step
    end
...
function message_send_task()
    parallel.waitForAll(chassis_speed_set)
    os.sleep(0.05)
end
...
while true do
    ...
    --message_send_task()
    parallel.waitForAll(chassis_control_task, message_send_task)
