...
function yaw_control()
    ...
    --print(gimbal.yaw_motor.cur_spd)
    
    gimbal.yaw_motor.cur_ang = yaw.getAngle()
    gimbal.yaw_motor.tar_ang = target.ang.yaw
    gimbal.yaw_motor.cur_spd = -yaw.getAngularVelocity()

    if gimbal.yaw_motor.tar_ang-attitude.cannon_ang_imu.yaw > pi then   -- 选择劣弧（最短）路径
        attitude.cannon_ang_imu.yaw = attitude.cannon_ang_imu.yaw + 2*pi
    elseif gimbal.yaw_motor.tar_ang-attitude.cannon_ang_imu.yaw < -pi then
        attitude.cannon_ang_imu.yaw = attitude.cannon_ang_imu.yaw - 2*pi
    end
