...
function message_receive_task()
    ...
    if senderChannel == 4 and message ~= nil then
        msg.mode,msg.yaw_ang,msg.pitch_ang,msg.mode,msg.fire_permit,msg.chassis_yaw_spd = message:match("(-?%d+) (-?%d+) (-?%d+) (-?%d+) (-?%d+)")
        attitude.chassis_imu_yaw_spd = msg.chassis_yaw_spd
        print(msg.yaw_ang)
    else 
