...
function attitude_calc_task()
    ...
    attitude.imu_pos = imu.getPosition()
    attitude.cannon_ang_world = mat_to_euler(attitude.rotate_matrix_cannon)
    ...
    print(attitude.cannon_ang_world.yaw)
end
...
function feed_forward()
    local y_accel_feed
    local z_accel_feed 
    if math.abs(attitude.imu_accel.y) > 0.3 then
        y_accel_feed = attitude.imu_accel.y * gimbal.m * math.cos(attitude.cannon_ang_imu.pitch)
    else 
        y_accel_feed = 0
    end

    if math.abs(attitude.imu_accel.z) > 0.3 then
        z_accel_feed = attitude.imu_accel.z * gimbal.m * math.sin(attitude.cannon_ang_imu.pitch)
    else
        z_accel_feed = 0
    end

    --local y_accel_feed = attitude.imu_accel.y * gimbal.m * math.sin(attitude.cannon_ang_imu.pitch)
    --local z_accel_feed = attitude.imu_accel.z * gimbal.m * math.cos(attitude.cannon_ang_imu.pitch)
